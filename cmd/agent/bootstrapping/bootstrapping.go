...
func (pf *policyfile) QueryURL() (string, error) {
	...
	u, err := url.Parse(pf.DownloadURL)
	if err != nil {
...
func downloadPolicyfiles(ctx context.Context, svc *api.ServerAPI, bsProcess *bootstrappingProcess) error {
	logger.DebugFuncInfo()
...
func getFunctionForChunksBootstrappingLog(svc *api.ServerAPI) func(chunk string) error {
	fn := func(chunk string) error {
		...
		err := agent.Retry(retriesNumber, time.Second, func() error {
			...
			_, statusCode, err := svc.ReportBootstrappingLog(cmd.GetContext(), &commandIn)
			switch {
			// 0<100 error cases??
			case statusCode == 0:
...
func processPolicyfiles(svc *api.ServerAPI, bsProcess *bootstrappingProcess) error {
	logger.DebugFuncInfo()
	for _, bsPolicyfile := range bsProcess.policyfiles {
...
func reportAppliedConfiguration(svc *api.ServerAPI, bsProcess *bootstrappingProcess) error {
	logger.DebugFuncInfo()
	...
	return svc.ReportBootstrappingAppliedConfiguration(cmd.GetContext(), &payload)
}
