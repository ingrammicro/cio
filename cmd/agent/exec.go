...
func ExecCode(code string, path string, filename string,
...
func createCommandWithFilename(command string) (cmd *exec.Cmd, cmdFileName string) {
	cmdFileName = strings.Join([]string{time.Now().Format(TimeLayoutYYYYMMDDHHMMSS), "_", utils.RandomString(10)}, "")
	if runtime.GOOS == "windows" {
...
func RunTracedCmd(command string,
