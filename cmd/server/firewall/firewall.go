...
func cmdAdd() error {
	log.Debugf(CurrentFirewallDriverDebugTrace, driverName())
	return FirewallRuleAdd()
}

func cmdUpdate() error {
	log.Debugf(CurrentFirewallDriverDebugTrace, driverName())
	return FirewallRulesUpdate()
}

func cmdRemove() error {
	log.Debugf(CurrentFirewallDriverDebugTrace, driverName())
	return FirewallRuleRemove()
}

// FirewallPolicyGet subcommand function
func FirewallPolicyGet() *types.Policy {
...
func FirewallRuleCheck() (*types.Policy, *types.PolicyRule, bool) {
	logger.DebugFuncInfo()
...
func FirewallRuleAdd() error {
	logger.DebugFuncInfo()
	svc, _, formatter := cmd.WireUpAPI()

	_, newRule, exists := FirewallRuleCheck()
	if exists == false {
...
func FirewallRulesUpdate() error {
	logger.DebugFuncInfo()
	svc, _, formatter := cmd.WireUpAPI()

	params, err := utils.FlagConvertParamsJSON(cmd.Rules)
	if err != nil {
...
func FirewallRuleRemove() error {
	logger.DebugFuncInfo()
	svc, _, formatter := cmd.WireUpAPI()

	policy, existingRule, exists := FirewallRuleCheck()
	if exists == true {
