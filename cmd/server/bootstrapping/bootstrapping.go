...
func init() {
	fLines := cmd.FlagContext{Type: cmd.Int, Name: cmd.Lines, DefaultValue: defaultThresholdLines, Shorthand: "l",
		Usage: "Maximum lines threshold per response chunk"}

	bootstrapCmd := cmd.NewCommand(cmd.RootCmd, &cmd.CommandContext{
		Use:   "bootstrap",
		Short: "Manages bootstrapping commands"},
	)
	cmd.NewCommand(bootstrapCmd, &cmd.CommandContext{
		Use:          "start",
		Short:        "Starts a bootstrapping routine to check and execute required activities",
		RunMethod:    Start,
		FlagContexts: []cmd.FlagContext{fLines}},
	)
	cmd.NewCommand(bootstrapCmd, &cmd.CommandContext{
		Use:       "stop",
		Short:     "Stops the running bootstrapping process",
		RunMethod: Stop},
	)
}
...
func Start() error {
	...
	return runBootstrapPeriodically(ctx, formatter)
}

// Stop stops the bootstrapping process
func Stop() error {
	logger.DebugFuncInfo()
...
func runBootstrapPeriodically(ctx context.Context, formatter format.Formatter) error {
	...
	applyAfterIterations, thresholdLines, interval, splay := getBootstrappingConfigOrDefaults(config)
	...
	for {
		var updated bool
		blueprintConfig, updated, err = getBlueprintConfig(ctx, svc, blueprintConfig, formatter)
		if err == nil {
			if updated || lastPolicyfileApplicationErr != nil ||
				...
				config, configUpdated, err = configuration.ReloadConfig()
		...
		// instance)
		ticker := time.NewTicker(time.Duration(interval+r.Intn(splay)) * time.Second)
...
func runBootstrapOnce(ctx context.Context, config *configuration.Config, formatter format.Formatter) error {
	_, thresholdLines, interval, splay := getBootstrappingConfigOrDefaults(config)

	svc, _, formatter := cmd.WireUpAPI()
	blueprintConfig, _, err := getBlueprintConfig(ctx, svc, nil, formatter)
	if err == nil {
		err = applyPolicyfiles(ctx, svc, blueprintConfig, formatter, thresholdLines)
	}
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
...
func getBootstrappingConfigOrDefaults(
	...
	thresholdLines = viper.GetInt(cmd.Lines)
	if thresholdLines <= 0 {
...
func getBlueprintConfig(
	...
	blueprintConfig, status, err := svc.GetBootstrappingConfiguration()
...
func applyPolicyfiles(
	...
	err = processPolicyfiles(svc, bsProcess)
	// Finishing time
	...
	reportErr := reportAppliedConfiguration(svc, bsProcess)
	if reportErr != nil {
...
func downloadPolicyfiles(ctx context.Context, svc *api.IMCOClient, bsProcess *bootstrappingProcess) error {
	logger.DebugFuncInfo()
	...
	for _, bsPolicyfile := range bsProcess.policyfiles {
		...
		_, status, err := svc.DownloadFile(bsPolicyfile.DownloadURL, tarballPath, false)
		if err == nil && status != 200 {
...
func preparePolicyfileCommand(
	bsProcess *bootstrappingProcess,
	bsPolicyfile policyfile,
...
func getFunctionForChunksBootstrappingLog(svc *api.IMCOClient) func(chunk string) error {
	fn := func(chunk string) error {
		...
		err := utils.Retry(retriesNumber, time.Second, func() error {
			...
			_, statusCode, err := svc.ReportBootstrappingLog(&commandIn)
			switch {
			// 0<100 error cases??
			case statusCode == 0:
...
func processPolicyfiles(svc *api.IMCOClient, bsProcess *bootstrappingProcess) error {
	logger.DebugFuncInfo()
	for _, bsPolicyfile := range bsProcess.policyfiles {
		...
		fn := getFunctionForChunksBootstrappingLog(svc)
...
func reportAppliedConfiguration(svc *api.IMCOClient, bsProcess *bootstrappingProcess) error {
	logger.DebugFuncInfo()
	...
	return svc.ReportBootstrappingAppliedConfiguration(&payload)
}
