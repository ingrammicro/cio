...
// Register registers the polling process
func Register() error {
...
func obtainServerKeys(config *configuration.Config) (rootCAcert string, cert string, key string, err error) {
	cs, err := api.NewIMCOClientWithCommandPolling(config)
	if err != nil {
		return
	}
