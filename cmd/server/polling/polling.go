...
func init() {
	...
	fLongTime := cmd.FlagContext{Type: cmd.Int64, Name: cmd.LongTime, DefaultValue: DefaultPollingPingTimingIntervalLong,
		Shorthand: "l",
...
func processingCommandRoutine(svc *api.IMCOClient, formatter format.Formatter, commandProcessed chan bool) {
	logger.DebugFuncInfo()

	// 1. Request for the new command available
	log.Debug("Retrieving available command")
	command, status, err := svc.GetNextCommand()
	if err != nil {
	...
	if status == 200 {
		...
		_, status, err := svc.UpdateCommand(command.ID, &commandIn)
		if err != nil {
