...
func CloudApplicationDeploymentList() error {
	...
	deps, err := svc.ListCloudApplicationDeployments()
	if err != nil {
...
func CloudApplicationDeploymentShow() error {
	...
	dep, _, err := svc.GetCloudApplicationDeployment(viper.GetString(cmd.Id))
	if err != nil {
...
func fillDeploymentInputs(formatter format.Formatter) map[string]interface{} {
	...
	if viper.IsSet(cmd.InputsFromFile) {
		caIn, err := utils.ConvertFlagParamsJsonFromFileOrStdin(viper.GetString(cmd.InputsFromFile))
		if err != nil {
	...
	if viper.IsSet(cmd.Inputs) {
		params, err := utils.FlagConvertParamsJSON(cmd.Inputs)
		if err != nil {
			formatter.PrintFatal("Cannot parse input attributes", err)
		}
...
func CloudApplicationDeploymentDeploy() error {
	...
	deploymentTask, err := svc.CreateCloudApplicationDeploymentTask(viper.GetString(cmd.Id), &deploymentIn)
	if err != nil {
	...
	log.Info("Deploying... ")
	for {
		deploymentTask, err = svc.GetCloudApplicationDeploymentTask(viper.GetString(cmd.Id), deploymentTask.ID)
		if err != nil {
...
func CloudApplicationDeploymentDelete() error {
	...
	deployment, err := svc.DeleteCloudApplicationDeployment(viper.GetString(cmd.Id))
	if err != nil {
	...
	for {
		deployment, status, err := svc.GetCloudApplicationDeployment(deploymentID)
		if err != nil {
			if status == 404 {
