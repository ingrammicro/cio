...
func LabelResolution(
	...
	for _, name := range labelNamesIn {
		...
		if (*labelIDsByName)[name] == "" {
			...
			newLabel, err := svc.CreateLabel(&labelPayload)
			if err != nil {
...
func LabelAdd() error {
	...
	labeledResources, err := svc.AddLabel(labelID, &labelIn)
	if err != nil {
...
func LabelRemove() error {
	...
	err := svc.RemoveLabel(labelID, viper.GetString(cmd.ResourceType), viper.GetString(cmd.Id))
	if err != nil {
