...
func CloudSpecificExtensionTemplateList() error {
	...
	csets, err := svc.ListCloudSpecificExtensionTemplates()
	if err != nil {
...
func CloudSpecificExtensionTemplateShow() error {
	...
	cset, err := svc.GetCloudSpecificExtensionTemplate(viper.GetString(cmd.Id))
	if err != nil {
		formatter.PrintFatal("Couldn't receive CSE template data", err)
	}
...
func CloudSpecificExtensionTemplateImport() error {
	...
	if viper.IsSet(cmd.DefinitionFromFile) {
		defIn, err := utils.ConvertFlagParamsJsonStringFromFileOrStdin(viper.GetString(cmd.DefinitionFromFile))
		if err != nil {
	...
	cseTemplate, err := svc.CreateCloudSpecificExtensionTemplate(&cseTemplateIn)
	if err != nil {
...
func CloudSpecificExtensionTemplateUpdate() error {
	...
	cseTemplate, err := svc.UpdateCloudSpecificExtensionTemplate(viper.GetString(cmd.Id), &cseTemplateIn)
	if err != nil {
...
func CloudSpecificExtensionTemplateListDeployments() error {
	...
	cseds, err := svc.ListCloudSpecificExtensionTemplateDeployments(viper.GetString(cmd.Id))
	if err != nil {
...
func CloudSpecificExtensionTemplateDelete() error {
	...
	err := svc.DeleteCloudSpecificExtensionTemplate(viper.GetString(cmd.Id))
	if err != nil {
