...
func RealmList() error {
	...
	realms, err := svc.ListRealms(viper.GetString(cmd.Id))
	if err != nil {
...
func RealmShow() error {
	...
	realm, err := svc.GetRealm(viper.GetString(cmd.Id))
	if err != nil {
...
func RealmNodePoolPlansList() error {
	...
	realms, err := svc.ListRealmNodePoolPlans(viper.GetString(cmd.Id))
	if err != nil {
		formatter.PrintFatal("Couldn't receive realms data", err)
	}
