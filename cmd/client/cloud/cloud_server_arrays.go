...
func init() {
	...
	fResourceType := cmd.FlagContext{Type: cmd.String, Name: cmd.ResourceType, DefaultValue: "server_array", Hidden: true,
		Usage: "Resource Type"}
...
func ServerArrayList() error {
	...
	serverArrays, err := svc.ListServerArrays()
	if err != nil {
...
func ServerArrayShow() error {
	...
	serverArray, err := svc.GetServerArray(viper.GetString(cmd.Id))
	if err != nil {
		formatter.PrintFatal("Couldn't receive server array data", err)
	}
...
func ServerArrayCreate() error {
	...
	if viper.IsSet(cmd.Labels) {
		serverArrayIn["label_ids"] = labels.LabelResolution(
	...
	serverArray, err := svc.CreateServerArray(&serverArrayIn)
	if err != nil {
...
func ServerArrayUpdate() error {
	...
	serverArray, err := svc.UpdateServerArray(viper.GetString(cmd.Id), &serverArrayIn)
	if err != nil {
...
func ServerArrayBoot() error {
	...
	serverArray, err := svc.BootServerArray(viper.GetString(cmd.Id))
	if err != nil {
...
func ServerArrayShutdown() error {
	...
	serverArray, err := svc.ShutdownServerArray(viper.GetString(cmd.Id))
	if err != nil {
...
func ServerArrayEmpty() error {
	...
	serverArray, err := svc.EmptyServerArray(viper.GetString(cmd.Id))
	if err != nil {
...
func ServerArrayEnlarge() error {
	...
	serverArray, err := svc.EnlargeServerArray(viper.GetString(cmd.Id), &serverArrayIn)
	if err != nil {
...
func ServerArrayServerList() error {
	...
	servers, err := svc.ListServerArrayServers(viper.GetString(cmd.Id))
	if err != nil {
...
func ServerArrayDelete() error {
	...
	err := svc.DeleteServerArray(viper.GetString(cmd.Id))
	if err != nil {
