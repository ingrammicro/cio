...
func ServerShow() error {
	...
	server, err := svc.GetServer(viper.GetString(cmd.Id))
	if err != nil {
		formatter.PrintFatal("Couldn't receive server data", err)
	}
...
func ServerCreate() error {
	...
	server, err := svc.CreateServer(&serverIn)
	if err != nil {
...
func ServerUpdate() error {
	...
	server, err := svc.UpdateServer(viper.GetString(cmd.Id), &serverIn)
	if err != nil {
...
func ServerBoot() error {
	...
	serverIn := map[string]interface{}{}
	server, err := svc.BootServer(viper.GetString(cmd.Id), &serverIn)
	if err != nil {
...
func ServerReboot() error {
	...
	serverIn := map[string]interface{}{}
	server, err := svc.RebootServer(viper.GetString(cmd.Id), &serverIn)
	if err != nil {
...
func ServerShutdown() error {
	...
	serverIn := map[string]interface{}{}
	server, err := svc.ShutdownServer(viper.GetString(cmd.Id), &serverIn)
	if err != nil {
...
func ServerOverride() error {
	...
	serverIn := map[string]interface{}{}
	server, err := svc.OverrideServer(viper.GetString(cmd.Id), &serverIn)
	if err != nil {
...
func ServerDelete() error {
	...
	err := svc.DeleteServer(viper.GetString(cmd.Id))
	if err != nil {
...
func ServerFloatingIPList() error {
	...
	floatingIPs, err := svc.ListServerFloatingIPs(viper.GetString(cmd.Id))
	if err != nil {
...
func ServerVolumesList() error {
	...
	volumes, err := svc.ListServerVolumes(viper.GetString(cmd.Id))
	if err != nil {
...
func EventsList() error {
	...
	events, err := svc.ListServerEvents(viper.GetString(cmd.Id))
	if err != nil {
...
func OperationalScriptsList() error {
	...
	scripts, err := svc.ListOperationalScripts(viper.GetString(cmd.Id))
	if err != nil {
...
func OperationalScriptExecute() error {
	...
	scriptOut, err := svc.ExecuteOperationalScript(viper.GetString(cmd.ServerId), viper.GetString(cmd.ScriptId), in)
	if err != nil {
