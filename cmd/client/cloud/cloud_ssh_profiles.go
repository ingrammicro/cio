...
func init() {
	...
	fResourceType := cmd.FlagContext{Type: cmd.String, Name: cmd.ResourceType, DefaultValue: "ssh_profile", Hidden: true,
		Usage: "Resource Type"}
...
func SSHProfileList() error {
	...
	sshProfiles, err := svc.ListSSHProfiles()
	if err != nil {
...
func SSHProfileShow() error {
	...
	sshProfile, err := svc.GetSSHProfile(viper.GetString(cmd.Id))
	if err != nil {
		formatter.PrintFatal("Couldn't receive sshProfile data", err)
	}
...
func SSHProfileCreate() error {
	...
	sshProfile, err := svc.CreateSSHProfile(&sshProfileIn)
	if err != nil {
...
func SSHProfileUpdate() error {
	...
	sshProfile, err := svc.UpdateSSHProfile(viper.GetString(cmd.Id), &sshProfileIn)
	if err != nil {
...
func SSHProfileDelete() error {
	...
	err := svc.DeleteSSHProfile(viper.GetString(cmd.Id))
	if err != nil {
