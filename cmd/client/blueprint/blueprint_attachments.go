...
func AttachmentShow() error {
	...
	attachment, err := svc.GetAttachment(viper.GetString(cmd.Id))
	if err != nil {
...
func AttachmentDownload() error {
	...
	realFileName, status, err := svc.DownloadFile(attachment.DownloadURL, viper.GetString(cmd.Filepath), false)
...
func AttachmentDelete() error {
	...
	err := svc.DeleteAttachment(viper.GetString(cmd.Id))
	if err != nil {
