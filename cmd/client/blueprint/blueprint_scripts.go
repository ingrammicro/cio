...
func ScriptShow() error {
	...
	script, err := svc.GetScript(viper.GetString(cmd.Id))
	if err != nil {
		formatter.PrintFatal("Couldn't receive script data", err)
	}
...
func ScriptCreate() error {
	...
	script, err := svc.CreateScript(&scriptIn)
	if err != nil {
...
func ScriptUpdate() error {
	...
	script, err := svc.UpdateScript(viper.GetString(cmd.Id), &scriptIn)
	if err != nil {
...
func ScriptDelete() error {
	...
	err := svc.DeleteScript(viper.GetString(cmd.Id))
	if err != nil {
...
func ScriptAttachmentAdd() error {
	...
	// adds new attachment
	attachment, err := svc.AddScriptAttachment(viper.GetString(cmd.Id), &attachmentIn)
	if err != nil {
		formatter.PrintFatal("Couldn't add attachment to script", err)
	}

	// uploads new attachment file
	err = svc.UploadFile(sourceFilePath, attachment.UploadURL)
	if err != nil {
	...
	attachment, err = svc.UploadedScriptAttachment(attachment.ID, &attachmentIn)
	if err != nil {
...
func cleanAttachment(attachmentID string) {
	svc, _, formatter := cmd.WireUpAPI()
	if err := svc.DeleteAttachment(attachmentID); err != nil {
...
func ScriptAttachmentList() error {
	...
	attachments, err := svc.ListScriptAttachments(viper.GetString(cmd.Id))
	if err != nil {
