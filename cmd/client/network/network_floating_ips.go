...
func init() {
	...
	fResourceType := cmd.FlagContext{Type: cmd.String, Name: cmd.ResourceType, DefaultValue: "floating_ip", Hidden: true,
		Usage: "Resource Type"}
...
func FloatingIPList() error {
	...
	floatingIPs, err := svc.ListFloatingIPs(viper.GetString(cmd.ServerId))
	if err != nil {
...
func FloatingIPShow() error {
	...
	floatingIP, err := svc.GetFloatingIP(viper.GetString(cmd.Id))
	if err != nil {
		formatter.PrintFatal("Couldn't receive floating IP data", err)
	}
...
func FloatingIPCreate() error {
	...
	floatingIP, err := svc.CreateFloatingIP(&floatingIPIn)
	if err != nil {
...
func FloatingIPUpdate() error {
	...
	floatingIP, err := svc.UpdateFloatingIP(viper.GetString(cmd.Id), &floatingIPIn)
	if err != nil {
...
func FloatingIPAttach() error {
	...
	server, err := svc.AttachFloatingIP(viper.GetString(cmd.Id), &floatingIPIn)
	if err != nil {
...
func FloatingIPDetach() error {
	...
	err := svc.DetachFloatingIP(viper.GetString(cmd.Id))
	if err != nil {
...
func FloatingIPDelete() error {
	...
	err := svc.DeleteFloatingIP(viper.GetString(cmd.Id))
	if err != nil {
...
func FloatingIPDiscard() error {
	...
	err := svc.DiscardFloatingIP(viper.GetString(cmd.Id))
	if err != nil {
