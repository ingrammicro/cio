...
func init() {
	...
	fResourceType := cmd.FlagContext{Type: cmd.String, Name: cmd.ResourceType, DefaultValue: "load_balancer", Hidden: true,
		Usage: "Resource Type"}
...
func LoadBalancerShow() error {
	...
	loadBalancer, err := svc.GetLoadBalancer(viper.GetString(cmd.Id))
	if err != nil {
...
func LoadBalancerUpdate() error {
	...
	loadBalancer, err := svc.UpdateLoadBalancer(viper.GetString(cmd.Id), &loadBalancerIn)
	if err != nil {
...
func LoadBalancerDelete() error {
	...
	loadBalancer, err := svc.DeleteLoadBalancer(viper.GetString(cmd.Id))
	if err != nil {
...
func LoadBalancerRetry() error {
	...
	loadBalancer, err := svc.RetryLoadBalancer(viper.GetString(cmd.Id), &loadBalancerIn)
	if err != nil {
...
func LoadBalancerPlanShow() error {
	...
	lbp, err := svc.GetLoadBalancerPlan(viper.GetString(cmd.Id))
	if err != nil {
