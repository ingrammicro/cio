...
//	GetDispatcherScriptCharacterizationsByType(ctx context.Context, phase string) (
//		scriptCharacterizations []*types.ScriptCharacterization, err error)
//	GetDispatcherScriptCharacterizationByUUID(ctx context.Context, scriptCharacterizationUUID string) (
//		scriptCharacterization *types.ScriptCharacterization, err error)
//	ReportScriptConclusions(ctx context.Context, scriptConclusions *map[string]interface{}) (
//		command *types.ScriptConclusion, status int, err error)
//	GetBootstrappingConfiguration(ctx context.Context) (
//		bootstrappingConfigurations *types.BootstrappingConfiguration, status int, err error)
//	ReportBootstrappingAppliedConfiguration(ctx context.Context,
//		bootstrappingAppliedConfigurationParams *map[string]interface{}) (err error)
//	ReportBootstrappingLog(ctx context.Context, bootstrappingContinuousReportParams *map[string]interface{}) (
//		command *types.BootstrappingContinuousReport, status int, err error)
//	GetPolicy(ctx context.Context) (policy *types.Policy, err error)
//	AddPolicyRule(ctx context.Context, ruleParams *map[string]interface{}) (policyRule *types.PolicyRule, err error)
//	UpdatePolicy(ctx context.Context, policyParams *map[string]interface{}) (policy *types.Policy, err error)
//	Ping(ctx context.Context) (ping *types.PollingPing, status int, err error)
//	GetNextCommand(ctx context.Context) (command *types.PollingCommand, status int, err error)
//	UpdateCommand(ctx context.Context, commandID string, pollingCommandParams *map[string]interface{}) (
//		command *types.PollingCommand, status int, err error)
//	ReportBootstrapLog(ctx context.Context, pollingContinuousReportParams *map[string]interface{}) (
//		command *types.PollingContinuousReport, status int, err error)
//	ObtainBrownfieldSslProfile(ctx context.Context, payload *map[string]interface{}) (
//		response map[string]interface{}, status int, err error)
//	ObtainPollingApiKey(ctx context.Context, payload *map[string]interface{}) (
//		response map[string]interface{}, status int, err error)
//	SetFirewallProfile(ctx context.Context, policyParams *map[string]interface{}) (
//		firewall *types.Firewall, status int, err error)
//	GetBrownfieldSettings(ctx context.Context) (settings *types.Settings, status int, err error)
//	SetBrownfieldSettings(ctx context.Context, payload *map[string]interface{}) (
...
func (imco *ServerAPI) ReportBootstrappingLog(ctx context.Context,
	...
	_, err = imco.PostAndCheck(ctx, pathBlueprintBootstrapLogs, bootstrappingContinuousReportParams, false, &command)
	if err != nil {
		return nil, status, err
	}
	return command, status, nil
...
func (imco *ServerAPI) RetrieveSecretVersion(ctx context.Context, svID, filePath string) (int, error) {
	...
	_, status, err := imco.DownloadFile(ctx, fmt.Sprintf(pathSecretVersionContent, svID), filePath, false)
	if err != nil {
		return status, err
	}
	return status, nil
